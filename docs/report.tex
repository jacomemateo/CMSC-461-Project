...
\setminted{
    bgcolor=codebg,
    breaklines,
    fontsize=\footnotesize,       % makes the code smaller
    baselinestretch=1.3    % increases line spacing a bit
}

\setstretch{1.2}

% SECTION NUMBERING FOR PRINT
...
\titleformat{\section}
  ...
  {\thesection} % Uses HW #1 in printed title
  {1em}
  {}
...
\renewcommand{\section}[1]{%
    \clearpage%
    % Add to TOC with normal numbering
    \addtocontents{toc}{\protect\setcounter{tocdepth}{2}}%
    \refstepcounter{section}% increment section counter
    \addcontentsline{toc}{section}{\arabic{section}\quad #1}% <-- TOC number only
    \oldsection*{HW \#\arabic{section}\quad #1}% <-- printed section
}
...
Here \verb|parking_type|, \verb|lot_type|, \verb|permit_type| and \verb|event_type| are all SQL enum types. I have defined them in \verb|db/migrations/01_enums.sql|
