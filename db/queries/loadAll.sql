INSERT VALUES INTO lot_info (name, lot_type) VALUES
    ('Lot A', 'STUDENT'),
    ('Lot B', 'STAFF'),
    ('Lot D', 'STUDENT'),
    ('Lot E', 'STAFF'),
...
INSERT VALUES INTO user_info (first_name, last_name, phone, email, parking_type) VALUES
...
INSERT VALUES INTO CAR_INFO (owner_id, plate, model, color) VALUES
...
INSERT INTO parking_spot (lot_id, space)VALUES
    ...
    ((SELECT id FROM lot_info WHERE name = 'Lot A'), 3),
    ...
    ((SELECT id FROM lot_info WHERE name = 'Lot C'), 3),
    ((SELECT id FROM lot_info WHERE name = 'Lot C'), 4),

    ((SELECT id FROM lot_info WHERE name = 'Lot D'), 1),
    ((SELECT id FROM lot_info WHERE name = 'Lot D'), 2);
...
(
    ...
    (SELECT ps.id
     ...
     WHERE l.name = 'Lot A' AND ps.space = 1),
    '2026-03-26 09:00:00-04',
    '2026-03-26 12:00:00-04'
),

(
    (SELECT id FROM user_info WHERE email = 'jason.chen@mit.edu'),
    (SELECT id FROM car_info WHERE plate = 'JAS567'),
    (SELECT ps.id
     FROM parking_spot ps
     JOIN lot_info l ON ps.lot_id = l.id
     WHERE l.name = 'Lot B' AND ps.space = 1),
    '2026-03-26 10:00:00-04',
    '2026-03-26 13:00:00-04'
),

(
    (SELECT id FROM user_info WHERE email = 'sophia.anderson@example.com'),
    (SELECT id FROM car_info WHERE plate = 'SOP890'),
    (SELECT ps.id
     FROM parking_spot ps
     JOIN lot_info l ON ps.lot_id = l.id
     WHERE l.name = 'Lot C' AND ps.space = 2),
    '2026-03-26 08:30:00-04',
    '2026-03-26 11:30:00-04'
);
...
INSERT INTO payments (user_id, amount, payment_time) VALUES
    ((SELECT id FROM user_info WHERE email = 'mateo.jacome@harvard.edu'), 1500, '2026-03-20 10:15:00-04'),
    ((SELECT id FROM user_info WHERE email = 'isaac.samuel@gmail.com'), 800, '2026-03-21 12:30:00-04'),
    ((SELECT id FROM user_info WHERE email = 'jason.chen@mit.edu'), 2000, '2026-03-22 09:45:00-04'),
    ((SELECT id FROM user_info WHERE email = 'lawrence.kemp@mit.edu'), 2500, '2026-03-22 14:00:00-04'),
    ((SELECT id FROM user_info WHERE email = 'james.taylor@example.com'), 1200, '2026-03-23 11:20:00-04'),
    ((SELECT id FROM user_info WHERE email = 'sophia.anderson@example.com'), 1800, '2026-03-24 08:10:00-04'),
    ((SELECT id FROM user_info WHERE email = 'william.thomas@example.com'), 2200, '2026-03-24 16:45:00-04'),
    ((SELECT id FROM user_info WHERE email = 'ava.martinez@example.com'), 900, '2026-03-25 13:05:00-04');
...
(
    (SELECT id FROM car_info WHERE plate = 'ISA456'),
    (SELECT ps.id
     FROM parking_spot ps
     JOIN lot_info l ON ps.lot_id = l.id
     WHERE l.name = 'Lot C' AND ps.space = 2),
    '2026-03-27 11:30:00-04',
    'EXPIRED_METER',
    TRUE
),

(
    (SELECT id FROM car_info WHERE plate = 'WIL901'),
    (SELECT ps.id
     FROM parking_spot ps
     JOIN lot_info l ON ps.lot_id = l.id
     WHERE l.name = 'Lot B' AND ps.space = 2),
    '2026-03-27 09:15:00-04',
    'INVALID_ZONE',
    FALSE
);
